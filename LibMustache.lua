--
-- LibMustache: WoW Wrapper for mustache.lua.
--

local MAJOR, MINOR = "LibMustache", 1

local LibMustache
if LibStub then
    LibMustache = LibStub:NewLibrary(MAJOR, MINOR)
    if not LibMustache then return end -- This version is already loaded.
else
    LibMustache = {}
end
...
---
--- glue.lua - https://raw.githubusercontent.com/luapower/glue
---
...
function glue.esc(s, mode) --escape is a reserved word in Terra
  s = s:gsub('%%','%%%%'):gsub('%z','%%z')
    :gsub('([%^%$%(%)%.%[%]%*%+%-%?])', '%%%1')
...
---
--- mustache.lua - https://raw.githubusercontent.com/luapower/mustache
---
